...
namespace Programmeringseksamens_projekt
{
    public class Board
    {
        ...
        public int LastPawnOrLastCaptureFlag;
        ...
        public void ApplyMove(Move move)
        {
            ...
            if (!_isSimulating)
            {
                if (move.Type == MoveType.CastlingQueenside || move.Type == MoveType.CastlingKingside) LastPawnOrLastCaptureFlag++;

                else if (move.Type == MoveType.Promotion || move.Type == MoveType.EnPassant) LastPawnOrLastCaptureFlag = 0;

                else if (move.Type == MoveType.Normal && Grid[move.From.row, move.From.col].Type != PieceType.Pawn && Grid[move.To.row, move.To.col] == null) LastPawnOrLastCaptureFlag++;

                else if (move.Type == MoveType.Normal && Grid[move.From.row, move.From.col].Type != PieceType.Pawn && Grid[move.To.row, move.To.col] != null) LastPawnOrLastCaptureFlag = 0;

                else if (move.Type == MoveType.Normal && Grid[move.From.row, move.From.col].Type == PieceType.Pawn) LastPawnOrLastCaptureFlag = 0;
            }

            Debug.Print($"{LastPawnOrLastCaptureFlag}");
            switch (move.Type)
            {
        ...
        public WinResult GetWinResult()
        {
            ...
            if (!IsInCheck(CurrentTurn) && LastPawnOrLastCaptureFlag == 50) return WinResult.Draw;

            if (PositionHistory.Values.Any(v => v >= 3)) return WinResult.Draw;
        ...
        private string GetPositionKey()
        {
            ...
            for (int row = 0; row < 8; row++)
                for (int col = 0; col < 8; col++)
                {
                    ...
                    Ps.Append(p == null ? "--" : $"{p.Color.ToString()[0]}{p.Type.ToString()[0]}");
                }
            Ps.Append(CurrentTurn == PieceColor.White ? 'W' : 'B');
            Ps.Append(WhiteCanCastleKingside ? '1' : '0');
            Ps.Append(WhiteCanCastleQueenside ? '1' : '0');
            Ps.Append(BlackCanCastleKingside ? '1' : '0');
            Ps.Append(BlackCanCastleQueenside ? '1' : '0');
